...
def train(rank, args, shared_model, optimizer=None):
    ...
    # Show training progress in Visdom
    episode = 0
    cum_reward_e = 0
    cum_reward_i = 0
    cum_r_e = []
    ...
    if rank == 0:
        vis = visdom.Visdom()
        vis.line(X=np.array([0]), Y=np.array([0]), win='train_cum_reward_e', opts=dict(title='Training Avg. external reward'))
        vis.line(X=np.array([0]), Y=np.array([0]), win='train_cum_reward_i', opts=dict(title='Training Avg. internal reward'))
    ...
    while True:
        ...
        for step in range(args.num_steps):
            ...
            cum_reward_e += reward
            cum_reward_i += reward_intrinsic

            reward += reward_intrinsic

            if done:
                episode += 1
                cum_r_e.append(cum_reward_e)
                cum_r_i.append(cum_reward_i)
                cum_reward_e = 0
                cum_reward_i = 0
        ...
        if rank == 0:
            if done and episode % args.log_steps == args.log_steps-1:
                vis.line(
                        X=np.array([log_step]),
                        Y=np.array([np.mean(cum_r_e)]),
                        win='train_cum_reward_e',
                        update='append')
                vis.line(
                        X=np.array([log_step]),
                        Y=np.array([np.mean(cum_r_i)]),
                        win='train_cum_reward_i',
                        update='append')
