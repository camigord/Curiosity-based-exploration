...
parser.add_argument('--max-episode-length', type=int, default=2100, metavar='M',
                    help='maximum length of an episode (default: 2100)')
...
parser.add_argument('--log_steps', type=float, default=10, metavar='LOG_STEPS',
                    help='Log performance after # steps (default: 10)')

parser.add_argument('--outdir', default="output/", help='Output log directory')
#parser.add_argument('--record', type=bool, default='False', help="Record the policy running video")
...
if __name__ == '__main__':
    ...
    if os.path.exists('output/model.pth'):
        print('Loading model...')
        shared_model.load_state_dict(torch.load('output/model.pth'))
